...
class ListSNNMulti(nn.Module):
    ...
    def calc_distill_loss(self, student_logits, teacher_logits):
        """
        ...
        """
        T = self.temp_distill
        kl_loss = nn.KLDivLoss()(F.log_softmax(student_logits/T, dim=1),
                                F.softmax(teacher_logits/T, dim=1)) * (T * T)
        self.distill_loss = kl_loss
            ...
            with torch.no_grad():
                ...
                if self.multi_model:
                    ...
                    if self.distill_mode:
                        self.loss = self.loss * (1. - self.alpha_distill) + self.distill_loss * (self.alpha_distill)
...
class ListSNNMulti_Teacher(nn.Module):
    """simple SNN model without SNNCell"""
    def __init__(self, model_config):
        super(ListSNNMulti_Teacher, self).__init__()
        print("SNNmodel_parallel instantiated")
        if hasattr(model_config, '__dict__'):
            self.__dict__.update(model_config.__dict__)
        else:
    ...
    def _init_layers(self):
        self.num_layer = np.size(self.teacher_network_size) - 1
        ...
        if self.multi_model:
            for m in range(self.num_models):
                if "x" in self.teacher_network_size[0]:
                    in_channels, height, width = [int(item) for item in self.teacher_network_size[0].split("x")]
                else:
                    in_channels = int(self.teacher_network_size[0])

                for l, layer_spec in enumerate(self.teacher_network_size[1:]):
                    if "conv" in layer_spec:
                        raise NotImplementedError
                    elif "fc" in layer_spec:
                        out_channels = int(layer_spec.strip("fc"))
            ...
            for layer_spec in self.teacher_network_size[1:]:
                if "conv" in layer_spec:
                    out_channels, kernel_size = [int(item) for item in layer_spec.strip("conv").split("c")]
                    padding = math.floor(kernel_size / 2)
                    layer = torch.nn.Conv2d(in_channels, out_channels, kernel_size,
        ...
        if self.target_type == 'train':
            ...
            with torch.no_grad():
                ...
                if self.multi_model:
                    ...
                    if self.distill_mode:
                        self.loss = self.loss * (1. - self.alpha_distill) + self.distill_loss * (self.alpha_distill)
                        # print(f"Normal Loss: {self.celoss + self.loss_nospike}, Distillation Loss: {self.distill_loss}")
